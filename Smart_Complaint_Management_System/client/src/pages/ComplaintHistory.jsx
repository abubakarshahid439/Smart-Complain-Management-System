...
import {
    Search, Filter, Menu, X, History, CheckCircle, XCircle
} from 'lucide-react';
...
const ComplaintHistory = () => {
    ...
    const { displayComplaints, allComplaints } = useMemo(() => {
        ...
        const historyList = fullList.filter(c => c.status === 'Resolved' || c.status === 'Rejected');

        let result = historyList.filter(c =>
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
            ...
            <p className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.3em] animate-pulse">Loading History...</p>
        </div>
    );
        ...
        <div className="flex h-screen w-full bg-gradient-to-br from-slate-50 via-white to-emerald-50/30 overflow-hidden">
            ...
            >
                ...
                <div className="w-full space-y-10 pr-4 lg:pr-8 py-10 pt-24">
                    <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 scroll-mt-24">
                        <div>
                            <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight font-outfit">
                                Complaint <span className="bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">History</span>
                            </h1>
                            <p className="text-slate-500 mt-2 font-medium">Archive of resolved and rejected complaints.</p>
                    ...
                    <div id="history" className="premium-card overflow-hidden bg-white/80 backdrop-blur-xl shadow-2xl border border-white/50 rounded-3xl">
                        <div className="p-6 sm:p-10 border-b border-emerald-50/50 flex flex-col md:flex-row md:items-center justify-between gap-6">
                            <div>
                                <h3 className="text-2xl font-black text-slate-900 font-outfit tracking-tight">Archived Tickets</h3>
                                <p className="text-slate-500 text-sm font-medium mt-1">Review past resolutions and closed cases.</p>
                            </div>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-gradient-to-r from-emerald-50/30 to-teal-50/30 text-emerald-600 text-[10px] uppercase font-black tracking-[0.2em]">
                                        ...
                                        <th className="px-10 py-6">Final Status</th>
                                        ...
                                        <tr>
                                            <td colSpan="5" className="px-10 py-20 text-center">
                                                <div className="flex flex-col items-center justify-center gap-4">
                                                    ...
                                                    <div>
                                                        <p className="text-lg font-extrabold text-slate-900 font-outfit">No History Found</p>
                                                        <p className="text-sm text-slate-500 font-medium">Archived complaints will appear here.</p>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
                                            ...
                                            >
                                                <td className="px-4 sm:px-10 py-6 sm:py-8">
                                                    <div className="flex items-center gap-3 sm:gap-4">
                                                        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-gradient-to-br from-emerald-100 to-teal-100 flex items-center justify-center text-emerald-600 font-black text-sm border border-emerald-200 group-hover:bg-white transition-colors">
                                                ...
                                                <td className="px-4 sm:px-10 py-6 sm:py-8">
                                                    <span className={clsx(
                                                        ...
                                                        c.status === 'Resolved' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-red-50 text-red-600 border-red-200'
                                                    )}>
                                                ...
                                                <td className="px-4 sm:px-10 py-6 sm:py-8 text-right">
                                                    <button className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-emerald-600 transition-colors group-hover:underline">
                                                        Details
                                                    </button>
                                                </td>
                                            </tr>
